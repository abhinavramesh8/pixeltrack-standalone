  ...
  namespace alpakatools {
    namespace device {
      namespace impl {
        ...
        class DeviceDeleter {
        public:
          DeviceDeleter(ALPAKA_ACCELERATOR_NAMESPACE::AlpakaDeviceBuf<TData> buffer) 
            : buf {std::move(buffer)} {}
      ...
      using unique_ptr = 
        ...
        std::unique_ptr<TData, impl::DeviceDeleter<std::conditional_t<
          allocator::policy == allocator::Policy::Caching, std::byte, TData>>>;
#else
    ...
    auto make_device_unique(const alpaka_common::Extent& extent) 
    {
      ...
      if constexpr (allocator::policy == allocator::Policy::Caching) {
        ...
        return typename device::unique_ptr<TData> {
          reinterpret_cast<TData*>(d_ptr), device::impl::DeviceDeleter<std::byte> {buf}};
      } else {
        ...
        return typename device::unique_ptr<TData> {
          d_ptr, device::impl::DeviceDeleter<TData> {buf}};
      }
#else
